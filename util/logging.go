...
// Got from https://raw.githubusercontent.com/codahale/http-handlers/master/logging/logging.go
// Could not send pr because repo is read-only
package util
...
// using it.
func LogWrap(h http.Handler, w io.Writer) *LoggingHandler {
...
func (al *LoggingHandler) ServeHTTP(w http.ResponseWriter, r *http.Request) {
	wrapper := &logWrapper{w: w, status: 200}
...
func (w *logWrapper) Header() http.Header {
	return w.w.Header()
}

func (w *logWrapper) Write(b []byte) (int, error) {
	return w.w.Write(b)
}

func (w *logWrapper) WriteHeader(status int) {
	w.status = status
...
func (w *logWrapper) Hijack() (net.Conn, *bufio.ReadWriter, error) {
